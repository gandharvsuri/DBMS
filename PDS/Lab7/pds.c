...
int get_rec_by_non_ndx_key(void *key, void *rec, int (*matcher)(void *rec, void *key))
{
    int status = PDS_REC_NOT_FOUND;
...
int delete_rec_by_ndx_key( int key )
{
    ...
    struct PDS_NdxInfo *index = (struct PDS_NdxInfo*)node->data;
    printf("!!!!!!!!!!!!!!!%d",index->is_deleted);
    if(index->is_deleted == 1)
    {
        printf("HERE\n");
        return status;
